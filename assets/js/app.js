// navlist
const NAV_DATA = ["HOME", "EVENTS", "FEED", "USER NAME"];

// card
const CARD_DATA = [
  {
    Image: "assets/image/webp/bar.webp",
    heading: "Lorem ipsum dolar sit de",
    date: "10/02/2022",
    city: "Mumbai",
    people: "1080",
    btn: "Go Check",
...
const footerData = [
  {
    title: "SERVICES",
    links: ["About", "News", "ullamcorper", "Contact"],
  },
  {
    title: "EVENTS",
    links: ["ullamcorper", "ullamcorper", "ullamcorper", "ullamcorper"],
  },
];
...
footer.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-[74px] ">

            ${footerData
              .map(
                (section) => `
                <div class="max-w-max">
                    <h3 class="font-bold max-w-max text-sm leading-[150%] text-white">
                        ${section.title}
                    </h3>

                    <ul class="flex flex-col gap-3 mt-[11px]">
                        ${section.links
                          .map(
                            (link) => `
                            <li class="text-white font-normal text-[12px]  ">
                                <a href="#" class="leading-[120%]">
                                    ${link}
                                </a>
                            </li>
                            `,
...
navList.innerHTML = NAV_DATA.map((item) => {
  return `<li class="font-normal leading-[125%] text-[16px] text-white hover:font-bold transform duration-300"><a href="#">${item}</a></li>`;
}).join("");
...
CARD_DATA.map((item, index) => {
  row.innerHTML += ` 
            ...
            class="border border-[#CCCCCC] lg:pt-[35px] lg:pr-[37px] lg:pb-[30px] lg:pl-[43px] p-7 rounded-[30px] group hover:shadow-[0px_8px_35px_0px_#0000001A] hover:border-transparent  flex-nowrap transition-all duration-500">
              ...
              <div>
              <div class=" font-normal text-base text-[#666666] leading-[150%] flex flex-row gap-2 items-center mt-3">
              ...
              ${item.date}</div>
              <div class=" font-normal text-base text-[#666666] leading-[150%] flex flex-row gap-2 items-center mt-[17px]">
              <span><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_129702_404)">
              ...
              ${item.city}</div>
              <div class=" font-normal text-base text-[#666666] leading-[150%] flex flex-row gap-2 items-center mt-[14px]">
              <span><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_129702_385)">
              ...
              <button class="bg-[#2C49FE] text-white border border-transparent hover:text-[#2C49FE] hover:bg-white hover:border-[#2C49FE] cursor-pointer duration-300 transition-all py-[11px] px-[28px]  text-base font-semibold leading-[150%] rounded-[23px] mt-[32px]">${item.btn}</button>
               </div>`;
});
